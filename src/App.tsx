...
// Load data asynchronously on mount
  useEffect(() => {
    ...
    fetchFromCloud(editMode).then((cloudData) => {
      if (cloudData && cloudData.length > 0) {
        ...
        if (!activeLessonId) setActiveLessonId(cloudData[0]?.lessons[0]?.id || '');
        setIsLoading(false);
      } else {
        ...
        getItem<Course[]>('teaching-toolkit-data').then((saved) => {
          if (saved && saved.length > 0) {
            setCourses(saved);
            if (!activeLessonId) setActiveLessonId(saved[0]?.lessons[0]?.id || '');
          } else {
            ...
            if (localSaved) {
              ...
              if (!activeLessonId) setActiveLessonId(parsed[0]?.lessons[0]?.id || '');
            } else {
              setCourses(mockCourses);
              if (!activeLessonId) setActiveLessonId(mockCourses[0]?.lessons[0]?.id || '');
            }
          }
          setIsLoading(false);
        }).catch(err => {
  ...
  const handleExportAllWord = async () => {
    ...
    try {
      ...
      } else if (courses.length > 0 && courses[0].title) {
        fileName = courses[0].title;
      }
      await exportAllToWord(courses, editMode, `${fileName}_所有單元.docx`);
    } catch (err) {
